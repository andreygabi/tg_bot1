...
async def com_addcontact(message: Message, bot: Bot, request: Request, state: FSMContext):
...
async def com_deletecontact(message: Message, bot: Bot, state: FSMContext):
...
async def com_viewcontacts(message: Message, bot: Bot, request: Request):
